...
The parser converts a `lay=""` attribute string into a structured `TokenNode[]`
array.
...
The parser walks the input string character by character using a single `while`
loop.
...
Scan forward until hitting whitespace, `:`, or `[`. Everything before that is
the key.

### Step 3: Standalone Key

If we hit whitespace or end-of-string after the key, push it as a standalone
flag.

### Step 4: Check for Colon

- **Colon found** token has a value or scope, continue to Step 5
- **No colon** unexpected character (e.g. `hover[...]`), push key as standalone,
...
If no `[` after `:`, read until whitespace. Colons inside values are allowed
(e.g. `bg:primary:hover` → value is `primary:hover`).
